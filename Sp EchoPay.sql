...
GO
GO
CREATE OR ALTER PROCEDURE GetMenuItemsByCategory
    @CategoryId INT
AS
BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_InsertTable
		@Table_Number INT,
		@Capacity INT,
		@Status NVARCHAR(50),
...
GO
GO
CREATE OR ALTER PROCEDURE sp_UpdateTable
		@Table_Id INT,
		@Table_Number INT,
		@Capacity INT,
		@Status NVARCHAR(50),
...
GO
GO
CREATE OR ALTER PROCEDURE sp_DeleteTable
		@Table_Id INT
	AS
	BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_GetTableById
		@Table_Id INT
	AS
	BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_UpdateTableStatus
		@Table_Id INT,
		@Status NVARCHAR(50)
	AS
	BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_DeleteSuppliers
    @Supplier_Id INT
AS
BEGIN
    SET NOCOUNT ON;
...
GO
GO
CREATE OR ALTER PROCEDURE DeleteInventory
    @InventoryID INT
AS
BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE GetInventoryById
    @InventoryID INT
AS
BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_GetShiftById
    @Shift_Id INT
AS
BEGIN
...
BEGIN
    ...
    SELECT @ExistingQuantity = Quantity FROM Cart WHERE Table_Id = @Table_Id AND Menu_Item_Id = @Menu_Item_Id;

    IF @ExistingQuantity IS NOT NULL
    BEGIN
        -- If it exists, update the quantity
        UPDATE Cart SET Quantity = Quantity + @Quantity WHERE Table_Id = @Table_Id AND Menu_Item_Id = @Menu_Item_Id;
    END
    ELSE
    BEGIN
        -- If it doesn't exist, insert a new record
        INSERT INTO Cart (Table_Id, Menu_Item_Id, Quantity, Price, User_Id)
        VALUES (@Table_Id, @Menu_Item_Id, @Quantity, @Price, 1);
    END
END;
GO
GO
CREATE OR ALTER PROCEDURE GetCartItemById
    @CartId INT
AS
BEGIN
    SELECT * FROM Cart
...
GO
GO
CREATE OR ALTER PROCEDURE sp_GetCartItemById
    @Cart_Id INT
AS
BEGIN
    SELECT * FROM Cart
...
GO
GO
CREATE OR ALTER PROCEDURE GetCartItemsByTable
			@Table_Id INT
		AS
		BEGIN
			SELECT 
...
GO
GO
CREATE OR ALTER PROCEDURE sp_ClearCart_NoUser
    @TableId INT
AS
BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_UpdateTablesStatus
    @Table_Id INT,
    @Status NVARCHAR(50)
AS
BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_GetOrderDetailsForBill
		@OrderId INT
	AS
	BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_InsertOrderFromCart
    @TableId INT,
    @UserId INT
AS
BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_MarkOrderAsReady
    @OrderId INT
AS
BEGIN
    UPDATE Orders
...
GO
GO
CREATE OR ALTER PROCEDURE UpdateOrderItemStatuss
    @Order_Item_Id INT,
...
GO
GO
CREATE OR ALTER PROCEDURE sp_GetDiscountIdByValue
...
GO
GO
CREATE OR ALTER PROCEDURE GetAllTaxes
AS
BEGIN
    SELECT * FROM Taxes;
END;
GO
GO
CREATE OR ALTER PROCEDURE GetTaxById
    @Tax_Id INT
AS
BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE AddTax
    @Name NVARCHAR(100),
...
GO
GO
CREATE OR ALTER PROCEDURE UpdateTax
    @Tax_Id INT,
    @Name NVARCHAR(100),
    @Percentage DECIMAL(5,2)
AS
...
GO
GO
CREATE OR ALTER PROCEDURE DeleteTax
    @Tax_Id INT
AS
BEGIN
...
GO
GO
CREATE OR ALTER PROCEDURE sp_GetTaxIdByPercentage
...
GO
GO
CREATE OR ALTER PROCEDURE GetLatestOrderIdByTable
    @Table_Id INT
AS
BEGIN
    SET NOCOUNT ON;
...
BEGIN
    ...
    UPDATE Orders SET Payment_Status_Id = @Payment_Status_Id WHERE Order_Id = @Order_Id;
...
GO
GO
CREATE OR ALTER PROCEDURE GetPaymentSummary
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @Today DATE = CAST(GETDATE() AS DATE);
    ...
    SELECT 
        ISNULL(SUM(CASE WHEN pm.Method_Name = 'Cash' THEN t.Amount ELSE 0 END), 0) AS CashTotal,
        ISNULL(SUM(CASE WHEN pm.Method_Name = 'UPI' THEN t.Amount ELSE 0 END), 0) AS UPITotal,
        ISNULL(SUM(CASE WHEN pm.Method_Name = 'Card' THEN t.Amount ELSE 0 END), 0) AS CardTotal,
        ISNULL(SUM(t.Amount), 0) AS GrandTotal
    FROM Transactions t
    INNER JOIN Payment_Method pm ON t.Payment_Method_Id = pm.Payment_Method_Id
    WHERE CAST(t.Transaction_Date AS DATE) = @Today;
END;
GO
